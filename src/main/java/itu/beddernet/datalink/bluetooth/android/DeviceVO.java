...
public class DeviceVO implements Runnable {
    ...
    public DeviceVO(long btAddress, BluetoothSocket conn, BluetoothDatalink bluetoothDatalink,
                    Observer packetHandlerIn, DeviceManager deviceManager) {
        ...
        try {
            ...
            Log.e(TAG, "Device connection error, input/output not established", e);
        }
...
//			reserved for datalink operations, message passed on if none of those
    ...
    public synchronized boolean write(byte[] pckt) {
        try {
            ...
            Log.e(TAG, "DeviceVO: did not manage to send packet:" + e.getMessage(), e);
            e.printStackTrace();
            deviceManager.handleBrokenConnection(btAddress);
            return false;
        }
