...
public class BluetoothDatalink implements DatalinkInterface {
    ...
    // This is hardcoded now, but should be based on device where possible like so:
     ...
     * surrounding devices
     *
     * @param rtrIncPcktObs   Observer in router layer to be notified about new incoming
     *                        packets
     * @param rtrNeighbourObs Observer in router layer to be notified about changes in
     *                        connected neighbours
     */
    public synchronized void connectToNetwork(Observer rtrIncPcktObs, Observer rtrNeighbourObs) {
        try {
     ...
     * list
     *
     * @param na   The address to send to
    ...
    public void connect(long dest) {
        try {
            ...
            BluetoothSocket conn = btAdapter.getRemoteDevice(addr).createRfcommSocketToServiceRecord(BT_NETWORK_UUID);
            conn.connect();
            Log.i(TAG, "BluetoothDatalink : connect called, will send to handle new connection");
    ...
    public void btStatus(boolean btStatus) {
        if (btStatus) {
            if (!idempotentGuard) {
                btAdd = NetworkAddress.castNetworkAddressToLong(btAdapter.getAddress());
                Log.d(TAG, "btAdd set");
