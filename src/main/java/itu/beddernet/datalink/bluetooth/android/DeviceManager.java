package itu.beddernet.datalink.bluetooth.android;
...
public class DeviceManager {
    ...
    public DeviceManager(Observer rtrIncPcktObs, Observer routerNeigbourObs, BluetoothDatalink bluetoothDatalink) {
    ...
    public synchronized void handleNewIncomingConnection(BluetoothSocket conn) {
        long bt = NetworkAddress.castNetworkAddressToLong(conn.getRemoteDevice().getAddress());
        DeviceVO device = new DeviceVO(bt, conn, bluetoothDatalink, packetHandlerIn, this);
     ...
     */
    public synchronized void handleNewlyDiscoveredConnections(Vector<BluetoothSocket> conns) {
        Log.d(TAG, "DeviceManager: Handle new connections called, conns size: " + conns.size());
        BluetoothSocket socket;
        Long bt;
        for (int i = 0; i < conns.size(); i++) {
            socket = conns.elementAt(i);
            bt = new Long(NetworkAddress.castNetworkAddressToLong(socket.getRemoteDevice().getAddress()));
            deviceStatus.put(bt, "Master");
            DeviceVO device = new DeviceVO(bt, socket, bluetoothDatalink, packetHandlerIn, this);
            new Thread(device).start();
            deviceTable.put(bt, device);
        }
        neighbourTableChanged();
     ...
     *
     * @param BTA the bt-address of the device with broken connection
     */
    ...
    private void neighbourTableChanged() {
        ...
        for (Enumeration<DeviceVO> e = deviceTable.elements(); e.hasMoreElements(); ) {
