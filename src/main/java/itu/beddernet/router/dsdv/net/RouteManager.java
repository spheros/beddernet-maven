...
import itu.beddernet.datalink.bluetooth.android.BluetoothDatalink;
...
public class RouteManager {
	 ...
	 *            appMsgObserver is the AppMessageObserver aka the applicaton that is being notified
     *
	 * @param dsdvRouter
	 */
	 ...
	 * 
	 * @param routes routes The vector of RoutingTableEntry objects to be sent
	 */
	...
	public void processDSDVMsgApplication(UniMessage appMsg) {
		...
		if (to == ConfigInfo.netAddressVal) {// message is intended for me
			...
			appMsgObserver.obtainMessage(appMsg.getType(), -1, -1, appMsg).sendToTarget();
	...
	public void processMultiAppMessage(MultiAppMessage multiAppMessage) {
		...
		for (long address : toAdresses) {
			if (address == myAddress) {// message is intended for me
				...
				// .sendToTarget();
				changed = true;
				appMsgObserver.obtainMessage(Message.MULTICAST_APPLICATION_MSG, -1, -1, multiAppMessage).sendToTarget();
				// change message)
				// sendMessage (redone)
			} else {
