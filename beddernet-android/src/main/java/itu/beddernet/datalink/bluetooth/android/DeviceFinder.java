...
public class DeviceFinder implements Runnable {
	...
	public void run() {
		...
		Log.d(TAG, "Found" + devicesDiscovered.size() + " devices - now trying to connect...");

		Thread connectionThread = new Thread(new connectThread(devicesDiscovered));
		...
		public void run() {
			...
			for (BluetoothDevice device : devices) {
				...
				if (dm.numberOfNeighbours() + newConnections.size() < BluetoothDatalink.MAX_OUT_DEGREE) {
					...
					if (!dm.exists(NetworkAddress.castNetworkAddressToLong(device.getAddress()))) {
						BluetoothSocket socket = null;
						// try {
						Log.d(TAG, "Trying to established connection to device:" + device.getAddress());
						try {
							socket = device.createRfcommSocketToServiceRecord(BT_NETWORK_UUID);
						} catch (IOException e) {
                            Log.e(TAG, "Devicefinder: could not create " + "socket to device", e);
						}
						try {
							socket.connect();
							Log.d(TAG,
							...
							try {
								...
								Log.e(TAG, "Exception fired with closing a socket" + e.getMessage());
							}
							socket = null;
							device = null; // 
