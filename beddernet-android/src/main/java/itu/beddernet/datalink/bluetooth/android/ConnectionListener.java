...
public class ConnectionListener extends Thread {
	...
	public ConnectionListener(DeviceManager dm) {
		try {
			...
			Log.e(TAG, "Error in ConnectionListener constructor:" + ex.getMessage());
			ex.printStackTrace();
		}
	}
	...
	public void run() {
		while (!Thread.interrupted() && aborted) {
			if (BluetoothAdapter.getDefaultAdapter().getState() != BluetoothAdapter.SCAN_MODE_CONNECTABLE_DISCOVERABLE) {
				Log.e(itu.beddernet.common.BeddernetInfo.TAG, "Not discoverable, requesting discoverable");
				Intent discoverableIntent = new Intent(BluetoothAdapter.ACTION_REQUEST_DISCOVERABLE);
				discoverableIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
				discoverableIntent.putExtra(BluetoothAdapter.EXTRA_DISCOVERABLE_DURATION, 300);
				if (BeddernetService.getBeddernetInstance() != null) {
					BeddernetService.getBeddernetInstance().startActivity(discoverableIntent);
				}
			}
			try {
				Thread.sleep(300000);
		...
		Log.i(itu.beddernet.common.BeddernetInfo.TAG, "Discoverable thread sucessfully interrupted");
	}
