...
public class PacketHandlerIn extends Thread implements Observer {
	...
	/**
	 * When a message is received by the device it is put in the incoming queue
	 * "incomingBuffer"
	 * 
	 * @param in
		...
		while (!abort) {
			try {
				...
				case Message.DSDV_FULLDUMP_MSG_CODE:
					RouteBroadcastMessage rBroad = new RouteBroadcastMessage(packet);
				...
				case Message.UNICAST_APPLICATION_MESSAGE_UAIH:
					UniAppMessageUAIH applicationServiceMessage = new UniAppMessageUAIH(packet);
